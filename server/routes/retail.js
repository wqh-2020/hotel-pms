...
module.exports = function(db) {
  ...
  router.post('/products', (req, res) => {
    try {
      ...
      db.prepare('INSERT INTO products(category_id,name,barcode,price,cost_price,unit,stock,description,image,is_available,sort_order) VALUES(?,?,?,?,?,?,?,?,?,?,?)').run(
        category_id || null, name, barcode || null, price || 0, cost_price || 0, unit || '个', stock || 0, description || null, image || null, is_available !== false ? 1 : 0, sort_order || 0
      )
      res.json({ code: 0, msg: '创建成功' })
    } catch (e) {
      console.error('[retail POST /products]', e.message)
      res.status(500).json({ code: 1, msg: '创建失败：' + e.message })
    }
  })
  router.put('/products/:id', (req, res) => {
    try {
      ...
      db.prepare('UPDATE products SET category_id=?,name=?,barcode=?,price=?,cost_price=?,unit=?,stock=?,description=?,image=?,is_available=?,sort_order=? WHERE id=?').run(
        category_id || null, name, barcode || null, price, cost_price, unit, stock, description || null, image || null, is_available ? 1 : 0, sort_order, req.params.id
      )
      res.json({ code: 0, msg: '更新成功' })
    } catch (e) {
      console.error('[retail PUT /products]', e.message)
      res.status(500).json({ code: 1, msg: '更新失败：' + e.message })
    }
  })
  ...
  router.post('/orders', (req, res) => {
    ...
    // 第一步：验证商品并补全价格/名称（统一从 DB 查，前端传的 price 作备用）
    const resolvedItems = []
    for (const item of items) {
      ...
      if (pid) {
        const prod = db.prepare('SELECT id, name, price FROM products WHERE id=?').get(pid)
        if (!prod) return res.json({ code: 1, msg: `商品ID ${pid} 不存在` })
        // 优先使用数据库中的价格，保证准确
        pname = prod.name
        price = Number(prod.price) || 0
      }
      resolvedItems.push({ pid: pid || null, pname: pname || '未知商品', price, qty })
    }

    // 第二步：计算金额
    let total = resolvedItems.reduce((s, i) => s + i.price * i.qty, 0)
    const payAmount = Math.max(0, total - discount_amount)
    const orderNo = genNo()
    ...
    for (const ri of resolvedItems) {
      db.prepare('INSERT INTO retail_order_items(order_id,product_id,product_name,product_price,quantity,subtotal) VALUES(?,?,?,?,?,?)').run(
        orderId, ri.pid, ri.pname, ri.price, ri.qty, ri.price * ri.qty
      )
      // 扣库存
      if (ri.pid) db.prepare('UPDATE products SET stock=MAX(0,stock-?) WHERE id=?').run(ri.qty, ri.pid)
    }
